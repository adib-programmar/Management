            ...
            <?php while ($student = $students->fetch_assoc()): ?>
                <div class="student-card bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                    <div class="p-6">
                        ...
                        <p class="text-gray-400 mb-4"><strong>Email:</strong> <?= htmlspecialchars($student['username']) ?></p>
                        <form method="POST" class="text-center">
                            <input type="hidden" name="delete_student_id" value="<?= $student['id'] ?>">
                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white text-sm font-bold py-2 px-4 rounded-full transition duration-300">
                                <i class="fas fa-user-minus mr-1"></i> Remove Student Account
                            </button>
                        </form>
                    </div>
                </div>
